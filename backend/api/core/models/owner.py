...
class Owner(models.Model):
    ...
    domain_type = models.CharField(
        max_length=16, choices=[(t.value, t.value) for t in DOMAINTYPE]
    )
    reception = models.OneToOneField(
        Reception, on_delete=models.PROTECT, null=True, blank=True
    )
    ...
    def clean(self):
        ...
        if non_null_fields != 1:
            raise IntegrityError(
                "Exactly one of reception, program, lab, or expert must be set."
            )
    ...
    class Meta:
        constraints = [
            ...
            CheckConstraint(
                condition=(
                    (
                        Q(reception__isnull=False)
                        & Q(program__isnull=True)
                        & Q(lab__isnull=True)
                        & Q(expert__isnull=True)
                    )
                    | (
                        Q(reception__isnull=True)
