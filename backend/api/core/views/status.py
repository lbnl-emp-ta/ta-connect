...
class StatusListView(views.APIView):
    ...
    def get(self, request, format=None):
        ...
        if maybe_context is None:
            return Response(
                data={"message": "Request is missing required context."},
        ...
        if request.user.id != context.get("user"):
            return Response(
                data={
                    "message": "User mismatch. Given user does not match logged in user."
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if context.get("role") is None:
            return Response(
                data={"message": "Missing role information in context."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                data={"message": "Given role does not exist"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if not has_role(request, role.name):
            return Response(
                data={"message": "User has not been assigned given role."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        return Response(
            data=RequestStatusSerializer(available_statuses, many=True).data,
