            ...
            with transaction.atomic():
                ...
                if not _org:
                    _org = Organization.objects.create(
                        ...
                        transmission_planning_region=TransmissionPlanningRegion.objects.get(
                            name=tpr
                        ),
                ...
                if not _customer.exists():
                    _customer = Customer.objects.create(
                        email=email, name=name, phone=phone, title=title
                    )
                else:
                ...
                if cohort:
                    ...
                    for participant in cohort:
                        CohortParticipant.objects.create(
                ...
                CustomerRequestRelationship.objects.create(
                    request=_request, customer=_customer, is_poc=True
                )

                create_audit_history(
                    request, _request, ActionType.StatusChange, "Request created"
                )
                create_audit_history(
                    request, _request, ActionType.Assignment, "Assigned to Reception"
                )
