...
class UploadAttachmentView(BaseUserAwareRequest):
    ...
    permission_classes = [permissions.IsAuthenticated, CanAddAttachment]
    ...
    def post(self, request, request_id):
        ...
        if not CanAddAttachment().has_object_permission(request, self, ta_request):
            return Response(
                data={
                    "message": "Insufficient authorization to upload attachment for given request"
                },
        ...
        if not request.data.get("file"):
            return Response(
                data={"message": "File to upload missing"},
        ...
        create_audit_history(
            ...
            f"Uploaded attachment: {attachment.title}",
        )

        return Response(
            data={"message": "Attachment successfully uploaded"},
...
class DownloadAttachmentView(BaseUserAwareRequest):
    ...
    def get(self, request, request_id, attachment_id):
        visible_requests = (
            self.get_actionable() | self.get_downstream() | self.get_inactive()
        )
        ...
        if not visible_requests.contains(ta_request):
            return Response(
                data={
                    "message": "Insufficient authorization to download attachment for given request"
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                data={"message": "Attachment with given filename does not exist"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class DeleteAttachmentView(BaseUserAwareRequest):
    ...
    def delete(self, request, request_id, attachment_id):
        ...
        if not CanDeleteAttachment().has_object_permission(request, self, ta_request):
            return Response(
                data={
                    "message": "Insufficient authorization to delete attachment for given request"
                },
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        create_audit_history(
            request,
            ta_request,
            ActionType.RemoveAttachment,
            f"Deleted attachment: {attachment_title}",
        )
...
class EditAttachmentView(BaseUserAwareRequest):
    ...
    def patch(self, request, request_id, attachment_id):
        ...
        try:
            ...
            return Response(
                data={
                    "message": "Attachment with given filename does not exist for given request"
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if not CanEditAttachment().has_object_permission(request, self, ta_request):
            return Response(
                data={
                    "message": "Insufficient authorization to edit attachment for given request"
                },
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        if "title" in body:
            if not body.get("title"):
                return Response(
                    data={"message": "Cannot clear title field"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        ...
        serializer = AttachmentEditSerializer(
            instance=attachment_obj, data=patch_data, partial=True
        )
        ...
        return Response(
            data=AttachmentSerializer(
                Attachment.objects.get(pk=attachment_obj.pk)
            ).data,
