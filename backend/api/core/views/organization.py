...
class OrganizationDetailView(views.APIView):
    ...
    def patch(self, request, organization_id):
        ...
        for ta_request in organization_requests:
            if CanEditOrganizationInfo().has_object_permission(
        ...
        if not has_permission:
            return Response(
                data={
                    "message": "Insufficient authorization to edit given organization's information"
                },
        ...
        if "transmission_planning_region" in request.data and (
            request.data.get("transmission_planning_region") is not None
        ):
            organization_patch_data["transmission_planning_region"] = request.data.get(
        ...
        serializer = OrganizationEditSerializer(
            organization_obj, data=organization_patch_data, partial=True
        )
    ...
    def delete(self, request, organization_id):
        ...
        if not CanDeleteOrganization().has_permission(request, self):
            return Response(
                data={
                    "message": "Insufficient authorization to delete given organization"
                },
...
class OrganizationCreateView(views.APIView):
    ...
    def post(self, request):
        ...
        if not CanCreateOrganization().has_permission(request, self):
            return Response(
                data={"message": "Insufficient authorization to create organization"},
...
class OrganizationTransferView(BaseUserAwareRequest):
    ...
    def post(self, request, request_id=None):
        ...
        if not request.data:
            return Response(
                data={"message": "Missing request body with organization_id"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if "organization_id" in request.data and (
            request.data.get("organization_id") is not None
        ):
            organization_id = request.data.get("organization_id")
        else:
            return Response(
                data={"message": "Missing organization_id in request body"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if not CanTransferOrganization().has_object_permission(
            request, self, ta_request
        ):
            return Response(
                data={
                    "message": "Insufficient authorization to transfer organization for given request"
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        return Response(
            data=OrganizationSerializer(organization_obj).data,
            status=status.HTTP_200_OK,
        )
