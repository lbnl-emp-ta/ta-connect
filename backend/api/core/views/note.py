...
class NoteCreateView(BaseUserAwareRequest):
    ...
    def post(self, request, request_id):
        ...
        if not CanAddNote().has_object_permission(request, self, ta_request):
            return Response(
                data={
                    "message": "Insufficient authorization to create note for given request"
                },
        ...
        if not "author" in self.request.data:
            return Response(
                data={"message": "Missing required author field"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if not "request" in self.request.data:
            return Response(
                data={"message": "Missing required request field"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if not "content" in self.request.data:
            return Response(
                data={"message": "Missing required content field"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        create_audit_history(
            ...
            ta_request,
            ActionType.AddNote,
            f"Added note: {response_note.content[:10]}...",
        )

        return Response(
            data=(NoteSerializer(response_note)).data, status=status.HTTP_200_OK
        )
...
class NoteDeleteView(BaseUserAwareRequest):
    ...
    def delete(self, request, request_id, note_id):
        ...
        if not CanDeleteNote().has_object_permission(request, self, ta_request):
            return Response(
                data={
                    "message": "Insufficient authorization to delete note for given request"
                },
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        try:
            ...
            return Response(
                data={"message": "Note with given ID does not exist"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        create_audit_history(
            ...
            f"Removed note: {note_obj.content[:10]}...",
        )
