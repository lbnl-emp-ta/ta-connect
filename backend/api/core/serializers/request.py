...
class RequestDetailSerializer(serializers.Serializer):
    ...
    status = serializers.SlugRelatedField(
        slug_field="name", required=False, queryset=RequestStatus.objects.all()
    )
    depth = serializers.SlugRelatedField(
        slug_field="name", required=False, queryset=Depth.objects.all()
    )
...
class RequestSerializer(serializers.Serializer):
    ...
    expert = serializers.SlugRelatedField(
        slug_field="email", required=False, allow_null=True, queryset=User.objects.all()
    )
    status = serializers.SlugRelatedField(
        slug_field="name",
        required=False,
    ...
    depth = serializers.SlugRelatedField(
        slug_field="name", required=False, allow_null=True, queryset=Depth.objects.all()
    )
    ...
    effort = serializers.CharField(
        max_length=25, required=False, allow_null=True, allow_blank=True
    )
    date_created = serializers.DateTimeField()
    ...
    def validate_proj_completion_date(self, value):
        ...
        if RequestSerializer.date_in_past(value):
            raise serializers.ValidationError(
                "Projected completion date cannot be in the past"
            )
        return value
    ...
    def validate(self, data):
        ...
        if (
            ...
            raise serializers.ValidationError(
                "Projected completion date must not be before projected start date"
            )
